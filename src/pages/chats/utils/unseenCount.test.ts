...
describe("countUnseenMessages", () => {
  it("counts messages newer than lastSeen from others (ownerPubkey aware)", () => {
    ...
    expect(
      countUnseenMessages({messages, lastSeenAtMs: 115_000, myPubKey: MY_PUBKEY})
    ).toBe(1)
  })
  ...
  it("returns 0 when messages or pubkey is missing", () => {
    expect(
      countUnseenMessages({messages: undefined, lastSeenAtMs: 0, myPubKey: MY_PUBKEY})
    ).toBe(0)
    expect(
      countUnseenMessages({messages: undefined, lastSeenAtMs: 0, myPubKey: ""})
    ).toBe(0)
  })
})
